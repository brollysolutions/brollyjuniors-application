...
// scrypt is memory-hard and ships with Node, so the app installs and runs with
// no native build step. Argon2id is the production upgrade; the stored format
// is versioned so a rehash-on-login migration is a small change.
...
export function tempPassword(len = 8): string {
