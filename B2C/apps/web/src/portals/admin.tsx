            ...
            <div className="card">
              ...
              {d.signupsByWeek.map((w: any) => (
                <div key={w.week} style={{ marginBottom: 8 }}>
                  ...
                  <Bar v={Math.min(100, w.enrolments * 4)} />
                </div>
              ))}
            </div>
            <div className="card">
  ...
  return (
    <Modal title={course ? `Edit ${course.title}` : 'New course'} onClose={onClose}>
      ...
      <div className="grid g3" style={{ gap: 10 }}>
        ...
        <Field label="Hours"><input value={f.durationHours} onChange={set('durationHours')} /></Field>
        <Field label="Price in paise" help={`Shows as ${price(Number(f.priceMinor) || 0)}`} error={err}>
          <input value={f.priceMinor} onChange={set('priceMinor')} />
        </Field>
      </div>
        ...
        <Action label={course ? 'Save changes' : 'Create as draft'} onClick={async () => {
          ...
          try {
            if (course) {
              ...
              await client.post('/admin/courses', { ...f, durationHours: Number(f.durationHours), priceMinor: Number(f.priceMinor) })
            }
            onDone()
              ...
              <div>
                <div className="card" style={{ marginBottom: 14 }}>
                  ...
                  <div className="editor">
                    ...
                    <textarea value={source} onChange={e => setDraft(e.target.value)} style={{ minHeight: 340 }} />
                  </div>
                </div>
...
function AddTeacher({ courses, onClose, onDone }: { courses: any[]; onClose: () => void; onDone: (r: any) => void }) {
  ...
  return (
    <Modal title="Add a teacher" onClose={onClose}>
      ...
      <Field label="Email"><input value={f.email} onChange={set('email')} placeholder="name@brollyjuniors.com" /></Field>
      ...
      <Field label="Years of experience"><input value={f.yearsExp} onChange={set('yearsExp')} /></Field>
