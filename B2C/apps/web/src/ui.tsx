...
/**
 * The three primitives that express every role and tenant difference in the UI.
 * There is no `if (isB2B)` anywhere; there is only "may you", "is it on", and
...
export const Head = ({ title, sub, right }: { title: React.ReactNode; sub?: React.ReactNode; right?: React.ReactNode }) => (
...
export const Stat = ({ k, v, d, small }: { k: string; v: React.ReactNode; d?: React.ReactNode; small?: boolean }) => (
...
export const Bar = ({ v }: { v: number }) => {
  ...
  return (
    <div className="barrow">
      <div className="bar"><i className={pct < 35 ? 'low' : pct < 70 ? 'mid' : ''} style={{ width: `${pct}%` }} /></div>
      <span>{pct}%</span>
    </div>
  )
}

export const Pill = ({ tone = 'mute', children }: { tone?: 'ok' | 'wait' | 'bad' | 'mute'; children: React.ReactNode }) =>
  <span className={`pill ${tone}`}>{children}</span>

export const Note = ({ tone, children }: { tone?: 'teal' | 'rose'; children: React.ReactNode }) =>
...
export function Empty({ title, detail, action }: { title: string; detail?: string; action?: React.ReactNode }) {
  return (
    <div className="card" style={{ textAlign: 'center', padding: '34px 20px' }}>
      <h3>{title}</h3>
      {detail ? <div className="sub" style={{ marginTop: 6, maxWidth: '46ch', marginInline: 'auto' }}>{detail}</div> : null}
...
export function Action({ label, working, onClick, kind = 'gold', disabled, small, title }: {
  label: string; working?: string; onClick: () => void | Promise<void>
  kind?: 'gold' | 'ghost' | 'dark'; disabled?: boolean; small?: boolean; title?: string
}) {
  const [busy, setBusy] = useState(false)
  const cls = `btn ${kind === 'gold' ? 'gold' : kind === 'ghost' ? 'ghost' : ''}${small ? ' sm' : ''}`
  return (
    <button className={cls} disabled={busy || disabled} title={title}
...
export function useLoad<T>(fn: () => Promise<T>, deps: unknown[] = []) {
  ...
  useEffect(() => {
    ...
    fn().then(
      ...
      err => { if (live) setState({ data: null, error: err.message ?? 'Could not load this.', loading: false }) },
    )
    return () => { live = false }
...
export function Page<T>({ q, children, what }:
  { q: { data: T | null; error: string | null; loading: boolean }; children: (d: T) => React.ReactNode; what?: string }) {
...
export const price = (minor: number) => formatPrice(minor, BROLLY_JUNIORS)
