        ...
        <>
          ...
          {d.courses.length === 0 ? (
            <Empty title="No courses yet"
              detail="Brolly assigns you to the courses you teach. Nothing appears here until they do." />
          ) : (
        ...
        <>
          ...
          <div className="filters" style={{ marginTop: 20 }}>
            {([['students', 'Students'], ['outline', 'Outline'],
              ...
              <button key={k} className={tab === k ? 'on' : ''} aria-pressed={tab === k}
              ...
              {d.students.map((s: any) => (
                <tr key={s.id} className="clickable" onClick={() => go('student', s.id)}>
                  ...
                  <td style={{ minWidth: 150 }}>
                    <Bar v={s.total_nodes ? s.done / s.total_nodes * 100 : 0}
                      label={`${s.full_name}'s progress`} />
                  </td>
                ...
                <div className="mod" key={m.id}>
                  ...
                  {m.lessons.map((l: any) => (
                    <div className="lessonrow locked" key={l.id}>
                      <span aria-hidden="true">▤</span><span>{l.title}</span><span className="spacer" />
                    </div>
                  ))}
                </div>
              ))}
    ...
    <Modal title="New assignment" onClose={onClose}>
      <Field label="Title">
        <input value={f.title} onChange={e => setF({ ...f, title: e.target.value })}
          placeholder="Build a marks calculator" />
      </Field>
      ...
      <div className="grid g3" style={{ gap: 10 }}>
        ...
        <Field label="Out of">
          <input value={f.maxScore} onChange={e => setF({ ...f, maxScore: e.target.value })} inputMode="numeric" />
        </Field>
    ...
    <Page q={q}>
      {(d: any) => (
        <>
          ...
          {d.students.length === 0
            ? <Empty title="Nobody yet" detail="Students appear here as they enrol in your courses." />
            : (
              ...
              <div>
                ...
                {s.code ? (
                  <div className="editor">
                    ...
                    <textarea readOnly value={s.code} aria-label="The code they submitted" />
                  </div>
                ) : null}
              </div>
              ...
              <div className="card">
                ...
                <table className="rubric">
                  <tbody>
                    {rubric.map(r => (
                      <tr key={r.key}>
                        <td><label htmlFor={`rb-${r.key}`}>{r.label}</label></td>
                        <td className="mono muted">of {r.max}</td>
                        <td>
                          <input id={`rb-${r.key}`} type="number" min={0} max={r.max} value={current[r.key]}
      ...
      {(d: any) => (
        <>
          ...
          {d.recordings.length === 0
            ? <Empty title="Nothing recorded yet" detail="Recordings appear here once Brolly publishes them." />
            : (
              <div className="lib">
                {d.recordings.map((r: any) => (
                  <div className="libcard" key={r.id}>
                    <div className="thumb"><div className="play" aria-hidden="true">▶</div>
